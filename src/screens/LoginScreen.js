...
const LoginScreen = () => {
  ...
  // Función de validación para verificar si el correo electrónico tiene el formato correcto
  const isValidEmail = (email) => {
    return /\b[A-Za-z0-9._%+-]+@(gmail|hotmail)\.com\b/.test(email);
  };
  ...
  const handleLogin = async () => {
    try {
      ...
      if (!isValidEmail(email)) {
        alert('El correo electrónico debe ser de Gmail o Hotmail');
        return;
      }
